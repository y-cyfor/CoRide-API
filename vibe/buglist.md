...
### BUG-026: 后端错误消息语言不统一
- **严重程度**: LOW
- **来源**: 测试报告 defect-report-2026-04-20.md
- **问题**: 后端 API 返回错误消息为英文（"Invalid username or password"），前端表单验证提示为中文（"请输入用户名"），用户体验不一致
- **影响**: 前端用户看到后端返回的英文错误时体验较差
- **状态**: ⏸️ 暂不修复 — 后端 API 使用英文是合理的（国际化 API 标准做法），理想方案是前端通过 i18n 映射后端错误码为中文，但改动范围较大。可在后续国际化完善时处理。

### BUG-027: 部分页面加载较慢
- **严重程度**: LOW
- **来源**: 测试报告 defect-report-2026-04-20.md
- **问题**: 用户中心、系统设置页面加载时间较长（10-20秒）
- **影响**: 用户体验差
- **状态**: ⏸️ 暂不修复 — 后端接口（如 get_system_config）仅读取内存配置无 DB 查询，延迟大概率是网络/环境因素（测试访问远程域名 https://coride.cyfor.cn/）。如需排查可检查：1) 前端首屏 bundle 大小 2) 浏览器 DevTools Network 面板耗时分布 3) 服务端并发请求阻塞。
