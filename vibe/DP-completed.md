...
| 33 | 渠道用量统计面板 | 中转层按 token/请求数统计渠道消耗，渠道列表显示累计/今日用量。纯 SQL 聚合，不依赖上游 API | 2026-04-18 |
| 34 | 请求缓存透传修复 | Anthropic 渠道自动添加 `prompt-caching-2024-07-31` beta header，确保上游缓存计费生效 | 2026-04-18 |
| 35 | 渠道配额可视化 | 渠道列表复用现有配额字段显示进度条 | 2026-04-18 |
| 36 | API Key 独立配额（渠道级） | `quotas` 表增加 `channel_id` 列，支持"用户 × 渠道"维度配额，优先渠道配额回退用户总配额 | 2026-04-18 |
| 37 | IP 访问控制 | 全局黑名单（系统设置）+ 用户白名单（用户管理），黑名单优先级最高，IP 提取 X-Real-IP > X-Forwarded-For > remote_addr | 2026-04-18 |
