...
启动成功后访问 `http://服务器IP`（或你映射的宿主机端口），使用你在 `.env` 中设置的管理员账号登录。

> **端口说明**：
> - Docker 容器内：nginx 监听 **80** 端口（反向代理到后端 8000 端口）
> - docker-compose 中 `"80:80"` 格式为 `"宿主机端口:容器端口"`
> - 如需更换外部访问端口，修改第一个数字即可，如 `"8080:80"` 则通过 `http://服务器IP:8080` 访问
> - **宝塔面板部署**：容器端口填 **80**，主机端口自定义（如 8080、9527 等），然后反向代理到 `http://127.0.0.1:主机端口`
